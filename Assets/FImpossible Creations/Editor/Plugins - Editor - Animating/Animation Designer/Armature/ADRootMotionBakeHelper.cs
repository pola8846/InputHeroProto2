...
namespace FIMSpace.AnimationTools
{
    ...
    public class ADRootMotionBakeHelper
    {
        ...
        public void PostRootMotion()
        {
            ...
            if (AnimationDesignerWindow._forceExportGeneric || AnimatorTransform.parent == null ) rootRefPos = startBakePos;

            Vector3 diff = RootMotionTransform.position - ( rootRefPos );
            Vector3 local = diff / ScaleOffset;

            if( AnimatorTransform.parent )
            {
                local = AnimatorTransform.InverseTransformVector( local );
            }
        ...
        public void SaveRootMotionPositionCurves(ref AnimationClip clip, string motionStr = "Motion", AnimationClip joinWith = null)
        {
            ...
            {
                if (_Bake_RootMPosX.length < _Original_RootMPosX.length ||
                    _Bake_RootMPosY.length < _Original_RootMPosY.length ||
                    _Bake_RootMPosZ.length < _Original_RootMPosZ.length ||
                    GetRootPositionCurvesMagnitude() < 0.001f) // If original root motion exists, use original root motion
                {
