...
namespace VolumetricFogAndMist {
    ...
    public partial class VolumetricFog : MonoBehaviour {
        ...
        const string VFM_BUILD_HINT = "VFMBuildHint120B1";
        ...
        float
            _lightDiffusionSpread = 8f;

        public float lightDiffusionSpread {
            get { return _lightDiffusionSpread; }
            set {
                if (value != _lightDiffusionSpread) {
                    _lightDiffusionSpread = value;
                    UpdateMaterialProperties();
                    isDirty = true;
                }
            }
...
#pragma warning restore 0162
            ...
            if (VRCheck.isVrRunning && fogRenderer.useSinglePassStereoRenderingMatrix) {
            ...
            if (_downsampling > 1f || _forceComposition) {
                ...
                if (!_edgeImprove || VRCheck.isVrRunning || SystemInfo.supportedRenderTargetCount < 2) {
            ...
            if (shiftToZero && VRCheck.isVrRunning) {
                mainCamera.ResetWorldToCameraMatrix();
            }
...
#pragma warning restore 0162
        ...
        void UpdateScatteringData(Camera mainCamera) {
            ...
            Vector3 viewportPos = mainCamera.WorldToViewportPoint(sunSkyPos, VRCheck.isVrRunning ? Camera.MonoOrStereoscopicEye.Left : Camera.MonoOrStereoscopicEye.Mono);
            if (viewportPos.z < 0) {
                if (isOrtho) {
                ...
                float diffusionIntensity = _lightScatteringDiffusion;
                if (_lightScatteringEnabled && LIGHT_DIFFUSION_ENABLED) {
                    diffusionIntensity *= LIGHT_DIFFUSION_MIE ? 0.1f * night : 1.2f * night * sunLightIntensity;
                }
                fogMat.SetVector(ShaderParams.FogScatteringData2, new Vector4(_lightScatteringIllumination, _lightScatteringDecay, _lightScatteringJittering, diffusionIntensity));
                float diffusionSpread = LIGHT_DIFFUSION_MIE ? _lightDiffusionSpread / 32.0001f: _lightDiffusionSpread;
                fogMat.SetFloat(ShaderParams.FogDiffusionSpread, diffusionSpread);
        ...
        async void UpdateTextureColors(Color[] colors, bool forceUpdateEntireTexture) {
            ...
            if (hasNewTextureData) {
                if (Application.isPlaying && _turbulenceStrength > 0f && adjustedChaosTexture != null && adjustedColors != null) {
