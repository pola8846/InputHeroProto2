...
#define VOLUMETRIC_FOG_INCLUDED
 ...
 void applyDiffusion(inout half4 sum) {
    half cosTheta = max( (dot( adir.xyz/adir.w, _SunDir)), 0.0 );
    #if defined(FOG_MIE_DIFFUSION)
        half sunAmount = MiePhase(cosTheta, _FogDiffusionSpread) * _FogScatteringData2.w;
    #else
        half sunAmount = pow(cosTheta * _FogScatteringData2.w, _FogDiffusionSpread);
    #endif
    sum.rgb += _SunColor.rgb * (sunAmount * sum.a);
 }
 #endif
